...
// Middleware
app.use(cors({
  origin:['http://localhost:5173','https://starlit-speculoos-f274d4.netlify.app','https://marathon-hub-12397.web.app','https://marathon-hub-12397.firebaseapp.com'],
  credentials:true
}));
...
async function run() {
  try {
    ...
    const registrationsCollection = client.db("marathonDB").collection("registrations");
...
//
    ...
    app.get("/marathons/by-email", verifyToken,  async (req, res) => {
      ...
      try {
        const result = await marathonCollection.find({ organizerEmail: email }).toArray();
        res.send(result);
      } catch (error) {
    ...
    // **GET**: Fetch registrations for a specific user
      ...
      try {
        const userRegistrations = await registrationsCollection.find(query).toArray();
